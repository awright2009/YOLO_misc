...
def save_point_cloud_ply(filename, vertices, colors):
    """
    Saves the point cloud (vertices and RGB colors) to a binary little-endian PLY file.
    """
    if vertices.shape[0] != colors.shape[0]:
        raise ValueError("Number of vertices and colors must match")

    num_vertices = vertices.shape[0]

    # Create the PLY header for binary format
    header = f"""ply
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print("Usage python DepthToCloud.py <rgb_file> <depth_file>\n")
        quit()
